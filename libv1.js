(function () {
    ...
    class libvil {
        ...
        init() {
            ...
            if (!document.getElementById('libvil-styles')) {
                ...
                style.textContent = `
                    ...
                    .libvil-scrollbar {
                        ...
                        scrollbar-color: #555 #2a2a2a; /* Firefox support */
                    }
            ...
            // Fixed: Use min-height and max-height for scrollable content area
            this.content = document.createElement('div');
            this.content.className = 'libvil-scrollbar';
            this.content.style.cssText = `
                padding: ${this.isMobile ? '8px' : '10px'};
                min-height: ${this.isMobile ? '200px' : '240px'};
                max-height: ${this.isMobile ? '260px' : '280px'};
                overflow-y: auto;
                overflow-x: hidden;
                background: #1a1a1a;
        ...
        createButton(name, script, folderId = null) {
            ...
            button.onclick = (e) => {
                e.preventDefault();
                this.executeScript(script, button);
            };

            if (folderId) {
                if (this.folders[folderId]) {
                    this.folderScripts[folderId][name] = script;
                    this.folders[folderId].scripts.push(name);
                } else {
            ...
            this.addLog(`Created button: ${name}${folderId ? ` in folder ${folderId}` : ''}`);
        }
        ...
        openFolder(folderId, folderName) {
            ...
            for (const [key, value] of Object.entries(items)) {
                if (value.type === 'folder') {
                    ...
                    button.onclick = (e) => {
                        e.preventDefault();
                        this.executeScript(value, button);
                    };
        ...
        executeScript(script, button) {
            if (!script) {
                this.addLog('Error: No script provided');
                return;
            }
            ...
            setTimeout(() => {
                try {
                    if (typeof script === 'function') {
                        script();
                        this.addLog(`Script "${button?.title || 'unknown'}" executed successfully`);
                    } else if (typeof script === 'string') {
                        eval(script);
                        this.addLog(`Script "${button?.title || 'unknown'}" executed successfully`);
                    } else {
                        this.addLog('Error: Invalid script type?');
                    }
                } catch (error) {
                    this.addLog(`Error executing script: ${error.message}`);
